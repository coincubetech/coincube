...
# checkout is <cache>/src and the build target is <cache>/target — siblings
# under whatever the cache dir resolves to, so no spelling of the cache dir
# puts Cargo's output inside the checkout. The target cannot be moved: an
# earlier revision let an environment variable relocate it and guarded the
# choice against landing in the checkout, but that guard compared path
# strings while containment is a property of the filesystem, and relative,
# `..`, symlink and case spellings each got past it (#396). Nothing here
# checks that <cache>/target is a plain directory: replace it with a symlink
# into the checkout and the next run refuses the dirty tree.
#
...
# The cache dir is used from inside the checkout later (`cd "$src" && …
# cargo build`), so a relative one would make Cargo write under
# "$src/<relative>/…" while the binary is looked for at "<cache>/target/…" —
# a full build that then "produced no binary", and a stray untracked tree
# inside the checkout that the next run's clean-tree check refuses. So the
# cache dir is created and resolved to its physical absolute path first, and
# the checkout and the target are derived from that one path.
mkdir -p "$cache_dir"
