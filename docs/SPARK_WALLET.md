...
- **Poisoned pools (why the bridge builds against a fork).** Flashnet pool creation is permissionless, and upstream SDK 0.19.0 quotes a V3 concentrated pool from its self-reported price alone, with no liquidity check — so an empty pool created with an absurd tick quotes a near-zero `amount_in`, wins `select_best_pool`'s 50%-weight fee-efficiency score against the real pool, and every conversion then fails at the AMM (`FSAG-4201`). This happened on mainnet BTC/USDB from 2026-09-07 (two zero-liquidity pools from host `auditlab`, prices 1e30 and 1e-30, one poisoning each direction) while the real pool was healthy. `coincube-spark-bridge/Cargo.toml` therefore pins `breez-sdk-spark` to `coincubetech/spark-sdk` branch `coincube/0.19.0-pool-selection-guard` — upstream 0.19.0 plus one commit in `crates/flashnet` that rejects pools whose reserve of the output asset can't cover the swap and drops pools priced more than 2× away from the deepest pool's. Reported to Breez; move back to an upstream tag once a release carries an equivalent fix. To inspect the live pool list: `curl 'https://api.flashnet.xyz/v1/pools?assetAAddress=<btc>&assetBAddress=<usdb-hex>'` with `<btc>` = `0202…02` (33 bytes) and `<usdb-hex>` the bech32m-decoded token id.
