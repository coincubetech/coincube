...
Spark ships **mainnet only**. The Breez Spark SDK (0.25.0 as of September 2026) does not publish a regtest flow, and COINCUBE uses real mainnet sats in small amounts for end-to-end testing. Liquid's regtest flow (`docs/BREEZ_SDK_REGTEST.md`) is untouched and still works.
...
Three things the toggle does *not* control, and how the app handles them:

- **Spending USDB.** The SDK only auto-attaches a USDB→BTC conversion to a sats send while the toggle is on. The bridge attaches the same conversion itself whenever the bitcoin balance falls short of `amount + fee` and USDB is held (`prepare_with_usdb_fallback` in `server.rs`), so the whole unified balance is spendable to any Bitcoin / Lightning / Spark destination regardless of the toggle. A send funded that way goes through the Flashnet AMM and carries a token leg (`has_token_leg`, no idempotency key); the Amount field says so once the entered amount exceeds the bitcoin balance. If the AMM refuses the conversion, the bridge answers with a `bad_request` naming the bitcoin balance that is still sendable and the AMM's reason, rather than the SDK's raw JSON.
- **Poisoned pools (why the SDK must stay ≥ 0.22.0).** Flashnet pool creation is permissionless, and SDK releases before 0.22.0 quoted a V3 concentrated pool from its self-reported price alone, with no reserve check — so an empty pool created with an absurd tick quoted a near-zero `amount_in`, won `select_best_pool`'s 50%-weight fee-efficiency score against the real pool, and every conversion failed at the AMM (`FSAG-4201`). This bit us on mainnet BTC/USDB from 2026-09-07 (two zero-liquidity pools from host `auditlab`, prices 1e30 and 1e-30, one poisoning each direction) while the real pool was healthy and while we were still on 0.19.0. Upstream commit `7ba9363a` ("Flashnet: validate pool data and swap responses", first in 0.22.0) added the output-reserve check for every curve plus price/tick/fee validation; the bridge moved to 0.25.0 on 2026-09-11. **The same afternoon a pool with `"hostName": null` appeared and 0.25.0 failed to parse the whole listing** (`Pool.host_name` is a `String`), so every conversion for every SDK client reported `NoPoolsAvailable`. The bridge therefore pins `breez-sdk-spark` to `coincubetech/spark-sdk` branch `coincube/0.25.0-stable-balance-hardening` — upstream 0.25.0 plus four commits proposed to Breez: parse the pool listing one pool at a time (skipping what doesn't parse) and allow a null host; drop pools priced >2× from the deepest pool's; back off failed `AutoConvert`/`Deactivation` conversions (30 s doubling to 1 h) instead of the every-6-seconds loop; emit `SdkEvent::StableBalanceConversionFailed` and persist a pending deactivation across restarts. The bridge's own circuit breaker still keys off the SDK's `Auto-conversion failed` warning so it works against an upstream build too. Drop back to an upstream tag once a release carries at least the listing fix. To inspect the live pool list: `curl 'https://api.flashnet.xyz/v1/pools?assetAAddress=<btc>&assetBAddress=<usdb-hex>'` with `<btc>` = `0202…02` (33 bytes) and `<usdb-hex>` the bech32m-decoded token id.
- **Where the toggle lives.** The SDK keeps it in its local object cache, not with the seed, so a Cube recovered on another machine comes up with it off while still holding USDB. The app records the user's decision in `CubeSettings::spark_stable_balance` and reconciles it against the SDK after the session's first `Synced`; with no recorded decision and USDB held, a banner points the user at the toggle. The decision is also mirrored to the Cube's Connect record (`sparkStableBalance` on `POST/PUT /connect/cubes`, echoed on every `CubeResponse`; same tri-state, absent-means-unchanged contract as `hasVault`): every local write is pushed, the registration request carries it, and a device with no decision adopts the server's when its registration answers — so a recovered Cube on a signed-in device picks the setting back up without the prompt.
- **The auto-conversion loop.** The SDK's `AutoConvert` worker has no backoff: a BTC→USDB swap that fails at the AMM (e.g. `Pool has no liquidity`) is refunded as an incoming Spark transfer, which re-queues the same conversion — one attempt every ~6 s, each a ±N-sat pair in the history. The SDK emits no event for it, so the bridge watches for the `Auto-conversion failed` warning with a `tracing` layer (`stable_balance_watch.rs`), and after `STABLE_BALANCE_PAUSE_THRESHOLD` (3) consecutive failures deactivates Stable Balance and emits `Event::StableBalancePaused`. The gui shows it as a banner and records the decision as "off" so the next launch doesn't re-arm it.
...
- **A stream of equal-and-opposite "Spark transfer" rows every few seconds** — the Stable Balance auto-conversion loop (see above). Look for `Auto-conversion failed` in the log; the reason after it (e.g. `Pool has no liquidity`) is the AMM's. The bridge pauses the feature on its own after three in a row; if the pairs continue, turn Stable Balance off in **Spark → Settings**.
- **"insufficient funds" on a send well under the shown balance** — the balance on the Send and Home cards is bitcoin *plus* USDB valued at the BTC/USD price. The `connected — identity …, balance N sats` line at bridge startup is bitcoin only. The bridge should cover the gap from USDB automatically; if it didn't, check whether `get_info` reports the USDB token balance.
