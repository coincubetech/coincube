...
fn internal_serialization_retains_all_maps_and_signing_intent() {
    ...
    assert_eq!(
        reparsed.inputs[0].sighash_type,
        Some(PsbtSighashType::from_u32(0x21))
    );
    assert!(reparsed.inputs[1].sighash_type.is_none());
    assert_eq!(reparsed.unknown, internal.unknown);
    assert_eq!(reparsed.proprietary, internal.proprietary);
    assert_eq!(reparsed.inputs[0].unknown, internal.inputs[0].unknown);
    assert_eq!(
        reparsed.inputs[0].proprietary,
        internal.inputs[0].proprietary
    );
    assert_eq!(reparsed.outputs[0].unknown, internal.outputs[0].unknown);
    assert_eq!(
        reparsed.outputs[0].proprietary,
        internal.outputs[0].proprietary
    );
    assert_eq!(
        reparsed.inputs[0].tap_internal_key,
        internal.inputs[0].tap_internal_key
    );
}
...
fn ordinary_psbt_without_unified_signatures_is_byte_content_preserving() {
    ...
    ordinary.inputs[1].tap_key_sig = Some(taproot_signature(8));
    let bytes = ordinary.serialize();
...
fn unified_taproot_sighash_is_not_enabled() {
    let mut ordinary = base_psbt();
    ordinary.inputs[0].tap_key_sig = Some(taproot_signature(8));
    let mut raw = RawPsbt::parse(&ordinary.serialize()).unwrap();
...
fn duplicate_complete_keys_are_rejected_in_every_map() {
    let raw = RawPsbt::parse(&mixed_internal_psbt().serialize()).unwrap();
...
fn standard_import_rejects_reserved_collisions_even_when_identical() {
    ...
    let internal_raw = RawPsbt::parse(&internal.serialize()).unwrap();
...
fn malformed_keys_der_and_unified_modes_are_explicit_errors() {
    ...
    assert!(matches!(
        import_standard(&invalid_key.serialize().unwrap()),
...
fn malformed_reserved_entries_fail_validation_and_export() {
    ...
    invalid_key.inputs[0].proprietary.insert(
        ProprietaryKey {
...
fn internal_legacy_and_unified_collision_is_rejected() {
    ...
    psbt.inputs[0].partial_sigs.insert(public_key, legacy);
    psbt.inputs[0]
        .proprietary
...
fn foreign_proprietary_namespaces_are_untouched() {
    ...
    assert_eq!(
        imported.inputs[0].proprietary.get(&foreign),
        Some(&vec![4, 5])
    );
...
fn version_and_typed_map_counts_fail_closed() {
    ...
    assert_eq!(
        import_standard(&version.serialize()),
        Err(UnifiedPsbtError::UnsupportedVersion(2))
    );
...
fn compact_size_truncation_nonminimality_and_trailing_data_are_rejected() {
    ...
    let mut trailing = base_psbt().serialize();
    trailing.push(0);
    assert_eq!(
...
fn truncated_fields_missing_maps_and_missing_transaction_are_rejected() {
    ...
    let mut missing_map = base_psbt().serialize();
    missing_map.pop();
    assert!(matches!(
...
fn checked_merge_is_idempotent_and_keeps_destination_metadata() {
    ...
    destination.inputs[0].unknown.insert(
        Key {
            type_value: 0x70,
            key: vec![1],
        },
    ...
    let metadata = destination.inputs[0].unknown.clone();

    let mut delta = base_psbt();
    let (legacy_key, legacy) = legacy_signature(15);
    delta.inputs[0].partial_sigs.insert(legacy_key, legacy);
    ...
    assert_eq!(destination.inputs[0].unknown, metadata);
    assert!(destination.inputs[0].partial_sigs.contains_key(&legacy_key));
...
fn merge_rejects_transaction_index_and_signature_conflicts_atomically() {
    let mut destination = base_psbt();
    let before = destination.serialize();
    let mut other_transaction = base_psbt();
    other_transaction.unsigned_tx.lock_time = absolute::LockTime::from_consensus(2);
    assert_eq!(
        merge_signatures(&mut destination, &other_transaction),
        Err(UnifiedPsbtError::UnsignedTransactionMismatch)
    );
    assert_eq!(destination.serialize(), before);
...
fn merge_rejects_legacy_unified_conflicts_and_is_atomic_across_inputs() {
    ...
    destination.inputs[1].partial_sigs.insert(key, legacy);
    let before = destination.serialize();

    let mut delta = base_psbt();
    let (addition_key, addition) = legacy_signature(20);
    delta.inputs[0].partial_sigs.insert(addition_key, addition);
...
fn mixed_internal_psbt() -> Psbt {
    let mut psbt = base_psbt();
    let (legacy_key, legacy) = legacy_signature(1);
    psbt.inputs[0].partial_sigs.insert(legacy_key, legacy);
    let (unified_key, unified) = unified_signature(2);
    psbt.inputs[0]
        .proprietary
        .insert(proprietary_key(&unified_key), unified);
    psbt.inputs[0].sighash_type = Some(PsbtSighashType::from_u32(0x21));
...
fn unified_signature_for_key(key_seed: u8, message_seed: u8) -> (PublicKey, Vec<u8>) {
    ...
    let digest = UnifiedSighashCache::new(&psbt.unsigned_tx, &prevouts)
        .unwrap()
        .signature_hash(
