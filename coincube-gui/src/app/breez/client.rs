...
impl BreezClient {
    /// Create a disconnected BreezClient for networks not supported by Breez SDK.
    /// All SDK operations will return a NetworkNotSupported error.
    ...
    pub fn liquid_signer(&self) -> Option<std::sync::Arc<std::sync::Mutex<HotSigner>>> {
        self.signer.clone()
    }
